...
//	template <typename... Arg> Batch(Arg&& ... arg):Parent(std::forward<Arg>(arg) ...){}
	Batch(){}
	Batch(std::vector<Vector>::iterator fir,std::vector<Vector>::iterator sec):std::pair<std::vector<Vector>::iterator,std::vector<Vector>::iterator>(fir,sec){}

	std::vector<Vector>::iterator begin() const {return first;}
	std::vector<Vector>::iterator end() const {return second;}
...
struct RBM
{
	...
	struct Conf
	{
		 ...
		 Conf():momentum_(0.5),weight_cost_(0.0002),learning_rate_(0.1){}
	};
	...
	RBM():type_(Type::SIGMOID){}
	RBM(size_t visible,size_t hidden):bias_visible_(visible),bias_hidden_(hidden),weight_(visible*hidden),bias_visible_inc_(visible),bias_hidden_inc_(hidden),weight_inc_(visible*hidden)
	{
		static std::default_random_engine eng(::time(NULL));
		static std::normal_distribution<float> rng(0.0,1.0);
	...
	int mirror (const RBM& rbm)
	{
		...
		if(n_hidden!=rbm.num_visible()||n_visible!=rbm.num_hidden())
			return -1;//here it means this cannot be mirrored
	...
	{
		size_t n_hidden=hidden.size(),n_visible=visible.size();
		std::fill(hidden,begin(),hidden.end(),0);
		for (size_t i = 0; i < n_hidden; i++)
		{
			float* xd=const_cast<float *>(weight.data()+i*n_visible);
			float s=v::dot(visible,v::LightVector(xd,xd+n+n_visible));
			s+=bias_hidden[i];
...
//    gw -= weight_ * weight_cost;
	...
	template <typename T> static void _read(std::istream& is,T& v)
	{
	...
	struct LRBM //layered rbm
	{
		...
		int build (const std::vector<int>& layers,const std::vector<int>& adjust=std::vector<int>())
		{
			...
			for (size_t i = 0; i < layers.size()-1; i++)
			{
				...
				//this is some new rbm with n_visible and n_hidden
		...
		void to_image(Vector& image,int& width,int& height)
		{
			...
			for(auto& rbm:rbms)
			{
				...
				{
					for(size_t j=0;j<n_hidden;j++)
						image[(y_offset+i)*width+x_offset+j]=rbm->weight_[j*n_hidden+i];
	...
	struct DeepBliefNet:public LRBM
	{
		...
		int train(std::vector<Vector>& inputs,std::vector<Vector>& labels,int max_layer,LRBM::Conf& conf,std::function<void(DeepBliefNet&)> progress=nullptr)
		{
			size_t n_samples=inputs.size(),n_labels=labels.size();
			int max_epoch=conf.max_epoch_,batch_size=conf.batch_size_,max_batches=std::min (conf.max_batches_,(int)(n_samples/batch_size));
			...
			for (int layer = 0; layer < max_layer; layer++)
			{
				...
				for(int epoch=0;epoch<max_epoch;epoch++)
				{
					...
					{
						int start=batch*batch_size,end=std::min(start+batch_size,(int)n_samples);
						Batch data;
						if(layer==0)
						...
						if(layer>0 && layer+1==max_layer-1&&!labels.empty())
						{
							...
							for(size_t i=start;i<end;i++)
							{
								...
								input.resize(input.size()+label_size);
		...
		int predict(const Vector& sample,Vector& output,Vector& probs)
		{
			...
			if(n_input +output.size() !=n_visible) return -1;
			...
			if(!output.empty())
			{
				Vector hs(n_visible),v2(n_visible);
		...
		struct GradientContext
		{
			...
			GradientContext(Batch inputs,std::vector<std::vector<Vector>> & probs,int epoch): max_iteration_(3),epoch_(epoch),
				inputs_(inputs),start_layer_(0),probs_(probs){}
		};
		...
		int gradient(GradientContext& ctx,const Vector& weights,Vector& weight_incs,float& cost)
		{
			...
			for(size_t sample=0;sample<n_samples; sample++)
			{
				...
				for (int layer = 0; layer < max_layer; layer++)
				{
					...
					{
						float* start=const_cast<float*> (rbm->bias_hidden_.data()),*end=start+rbm->num_weight();
		...
		int minimize(GradientContext& ctx)
		{
			...
			const int max_itereration=ctx.max_iteration_;
			...
			for(int i=0;i<max_itereration;++i)
			{
				float best_cost=f0;
				Vector best_weight(weights),best_weight_incs(weight_incs);
				float f3=0;
				...
				while(true)
				{
					...
					while(true)
					{
						...
						if(v::isfinite(cost)&&v::isfinite(weight_incs))
						{
							if(f3<best_cost)
							{
								best_cost=f3;
								best_weight=tmp_weights;
					...
					if(!v::isfinite(x3)||x3<0||x3>upper) x3=upper;
			...
			while((std::abs(d3) >-SIG*d0||f3>f0+x3*RHO*d0)&& M>0)
			{
				...
				if(f4>f0)
				{
					x3=x2-(0.5*d2*dx*dx)/(f4-f2-d2*dx);
				}else
				{
					float A=6*(f2-f4)/dx+3*(d4+d2);
			...
			{
				weights=best_weight;f0=best_cost;df0=best_weight_incs;
	...
	virtual int pretrain(std::vector<Vector>& inputs,LRBM::Conf& conf,std::function<void(DeepBliefNet&)> progress =nullptr)
	{
		static std::vector<Vector> nil;
		return train(inputs,nil,this->rbms_.size()-1,conf,progress);
	}
	int backprop(std::vector<Vector>& inputs,std::vector<Vector>& targets,LRBM::Conf& conf,std::function<void(DeepBliefNet&) > progress =nullptr)
	{
...
struct AutoEncoder:public DeepBliefNet
{
	int pretrain(std::vector<Vector>& inputs,LRBM::Conf& conf,std::function<void(DeepBliefNet&)> progress =nullptr)
	{
		int max_layer=this->rbms_.size();
		if(max_layer%2!=0)
			return -1;
		...
		static std::vector<Vector> nil;
		int ret=DeepBliefNet::train(inputs,nil,this->rbms_.size()/2,conf,progress);
		if(ret<0) return ret;
	...
	int backprop(std::vector<Vector> &inputs,LRBM::Conf& conf,std::function<void(DeepBliefNet&)> progress=nullptr)
	{
		static std::vector<Vector> nil;
		return DeepBliefNet::backprop(inputs,nil,conf,progress);
	}
};
